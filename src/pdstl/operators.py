...
class STL_Formula(torch.nn.Module):
    ...
    Unlike standard STL, inputs are belief trajectories and outputs are
    probability intervals [lower_bound, upper_bound].
    """
...
class Implies(STL_Formula):
    ...
    def robustness_trace(self, belief_trajectory, scale=-1, keepdim=True, **kwargs):
        return self.equivalent(belief_trajectory, scale=scale, keepdim=keepdim, **kwargs)
...
class Temporal_Operator(STL_Formula):
    ...
    Key insight: The bounds dimension [..., 2] is just carried through!
    All operations (min/max) work element-wise on it automatically.
    """
    ...
    def __init__(self, subformula, interval=None):
        ...
        self.steps = 1 if not self.interval else int(self._interval[-1] - self._interval[0] + 1)
...
class Until(STL_Formula):
    ...
    Discrete-time semantics (for each t):
      rob(t) = max_{τ ∈ [t+a, t+b]}  min( ψ(τ),
                                         min_{k ∈ [t, τ)} ϕ(k) )

    Here we implement this directly using Minish / Maxish on the
    [B,T,D,2] bounds traces of ϕ and ψ.

    - 'interval' is [a, b] with a, b >= 0, integers in discrete time.
    - If b = ∞, we cap it at T-1 for a finite horizon trace.
    """
        ...
        for t in range(T):
            ...
            for tau in range(start, end + 1):
                ...
                if tau == t:
                    ...
                    # min over time dim=1
                    min_phi = self.min_op(phi_segment, scale, dim=1, keepdim=False)  # [B,D,2]
