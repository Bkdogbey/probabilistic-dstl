...
class ObstacleAvoidance(STL_Formula):
    ...
    def robustness_trace(self, belief_trajectory, **kwargs):
        ...
        for t in range(len(belief_trajectory)):
            ...
            p_x_inside = self._normal_cdf((x_max_t - mu_x) / std_x) - self._normal_cdf((x_min_t - mu_x) / std_x)
            p_y_inside = self._normal_cdf((y_max_t - mu_y) / std_y) - self._normal_cdf((y_min_t - mu_y) / std_y)
            ...
            overlap_x = (lower_bound[:, :, 0:1] < x_max_t) & (upper_bound[:, :, 0:1] > x_min_t)
            overlap_y = (lower_bound[:, :, 1:2] < y_max_t) & (upper_bound[:, :, 1:2] > y_min_t)
            overlap = (overlap_x.float() * overlap_y.float())
            
            p_inside_pessimistic = torch.clamp(p_inside + 0.2 * overlap, 0.0, 1.0)
            p_outside_lower = torch.clamp(1.0 - p_inside_pessimistic, self.eps, 1.0 - self.eps)
            ...
            clearly_outside = (mean_outside_x.float() + mean_outside_y.float()).clamp(0, 1)
            
            p_inside_optimistic = p_inside * (1.0 - 0.2 * clearly_outside)
            p_outside_upper = torch.clamp(1.0 - p_inside_optimistic, self.eps, 1.0 - self.eps)
...
class GoalReaching(STL_Formula):
    ...
    def robustness_trace(self, belief_trajectory, **kwargs):
        ...
        for t in range(len(belief_trajectory)):
            ...
            # For rigorous treatment: σ_d² ≈ (∂d/∂x)²σ_x² + (∂d/∂y)²σ_y²
            # where ∂d/∂x = dx/d, ∂d/∂y = dy/d
            var_dist = (dx**2 / (dist_mean**2 + self.eps)) * var[:, :, 0:1] + \
                       (dy**2 / (dist_mean**2 + self.eps)) * var[:, :, 1:2]
